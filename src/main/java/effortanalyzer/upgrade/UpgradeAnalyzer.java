...
public class UpgradeAnalyzer {
    ...
    private void parseIbmOutput(Path jsonDir) throws IOException {
        ...
        try (Stream<Path> walk = Files.walk(jsonDir)) {
            jsonFiles = walk.filter(p -> p.toString().endsWith(".json")).toList();
        }
    ...
    private void writeInstructionsSheet(Workbook wb, Styles s) {
        ...
        addInstRow(sheet, s, r++, "Phase 1 — Java 21 (IBM)",
        ...
        addInstRow(sheet, s, r++, "Phase 2 — Libraries",
        ...
        addInstRow(sheet, s, r++, "Exclusions",
        ...
        addInstRow(sheet, s, r++, "🚫 Excluded Rules",
        ...
        addInstRow(sheet, s, r++, "HIGH  (orange background)",
        ...
        addInstRow(sheet, s, r++, "MEDIUM  (yellow background)",
        ...
        addInstRow(sheet, s, r++, "LOW  (green background)",
        ...
        addInstRow(sheet, s, r++, "Step 1 — Triage",
                "Open the 📊 Summary sheet. Identify components with HIGH-severity IBM findings.");
        addInstRow(sheet, s, r++, "Step 2 — Fix Java 21 Issues",
        ...
        addInstRow(sheet, s, r++, "Step 3 — Fix Library Issues",
    ...
    private void writeSummarySheet(Workbook wb, Styles s) {
        ...
        for (Map.Entry<String, List<IbmFinding>> entry : ibmFindingsByComponent.entrySet()) {
            ...
            Map<String, Long> bySev = findings.stream()
                    .collect(Collectors.groupingBy(IbmFinding::severity, Collectors.counting()));
            long high = bySev.getOrDefault("HIGH",   0L);
            long med  = bySev.getOrDefault("MEDIUM", 0L);
            long low  = bySev.getOrDefault("LOW",    0L);

            List<LibraryUpgradeAnalyzer.Finding> libFindings = spring.getFindingsByJar().entrySet().stream()
                    .filter(e -> e.getKey().contains(comp))
                    .flatMap(e -> e.getValue().stream())
                    .toList();
            long libCnt = libFindings.size();
            long total  = high + med + low + libCnt;

            // Factor in library finding severity alongside IBM severity
            String libPri = libraryPriority(libFindings);
            String pri = high > 0 ? "HIGH"
                    : "HIGH".equals(libPri) ? "HIGH"
                    : med > 0 ? "MEDIUM"
                    : "MEDIUM".equals(libPri) ? "MEDIUM"
                    : "LOW";
            ...
            CellStyle sevS = severityCellStyle(s, pri);
            if (sevS != null) { pc.setCellStyle(sevS); }
            if (high > 0) hc.setCellStyle(severityCellStyle(s, "HIGH"));
        }
        ...
        for (Map.Entry<String, List<LibraryUpgradeAnalyzer.Finding>> entry
            ...
            List<LibraryUpgradeAnalyzer.Finding> libFindings = entry.getValue();
            long libCnt = libFindings.size();
            String pri = libraryPriority(libFindings);
        ...
        // ── Totals row (single groupBy across all IBM findings) ───────────────
        r++;
        Row tot = sheet.createRow(r);
        Map<String, Long> totBySev = ibmFindingsByComponent.values().stream()
                .flatMap(List::stream)
                .collect(Collectors.groupingBy(IbmFinding::severity, Collectors.counting()));
        long tH = totBySev.getOrDefault("HIGH",   0L);
        long tM = totBySev.getOrDefault("MEDIUM", 0L);
        long tL = totBySev.getOrDefault("LOW",    0L);
    ...
    private void writeJava21Sheet(Workbook wb, Styles s) {
        ...
        for (Map.Entry<String, List<IbmFinding>> entry : ibmFindingsByComponent.entrySet()) {
            ...
            for (IbmFinding f : sorted) {
                Row row = sheet.createRow(r++);
                CellStyle sev = severityCellStyle(s, f.severity());
    ...
    /**
     * Maps library finding severities (CRITICAL / WARNING / INFO) to the summary
     * priority levels (HIGH / MEDIUM / LOW) used in the Summary sheet.
     * CRITICAL library findings elevate priority to HIGH; WARNING to MEDIUM.
     */
    private static String libraryPriority(List<LibraryUpgradeAnalyzer.Finding> findings) {
    ...
    private static CellStyle severityCellStyle(Styles s, String severity) {
        return s.severityStyle(severity);
    }
