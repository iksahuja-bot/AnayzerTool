...
public class ReportAnalyzer {
     ...
     *
     * JSON serialization is performed once per result in a pre-pass that also
     * determines the maximum number of detail columns — avoiding the previous
     * double-serialization where calculateMaxDetailColumns and writeDataRow each
     * called the ObjectMapper independently.
     */
    private void writeExcelOutput() throws IOException {
        ...
        // Pre-serialize detail chunks once for all results
        record SerializedResult(GroupedResult gr, List<String> chunks) {}
        int maxDetailColumns = 1;
        List<SerializedResult> serialized = new ArrayList<>(groupedResults.size());
        for (GroupedResult gr : groupedResults.values()) {
            ...
            try {
                ...
                chunks = ExcelUtils.splitIntoChunks(json, config.getMaxCellLength());
            } catch (IOException e) {
                logger.warn("Failed to serialize details for: {}", gr.getRuleId());
                chunks = List.of("");
            }
            serialized.add(new SerializedResult(gr, chunks));
            maxDetailColumns = Math.max(maxDetailColumns, chunks.size());
        }

        try (Workbook wb = new XSSFWorkbook()) {
            Sheet sheet = wb.createSheet("Results");

            // Header row — fixed columns first, then detail chunk columns
            Row headerRow = sheet.createRow(0);
            CellStyle headerStyle = ExcelUtils.createHeaderStyle(wb);
            for (int i = 0; i < FIXED_HEADERS.length; i++) {
                Cell c = headerRow.createCell(i);
                c.setCellValue(FIXED_HEADERS[i]);
                c.setCellStyle(headerStyle);
            }
            for (int i = 1; i <= maxDetailColumns; i++) {
                Cell c = headerRow.createCell(4 + i);
                c.setCellValue("MergedDetails_JSON_Part" + i);
                c.setCellStyle(headerStyle);
            }

            // Data rows — use pre-serialized chunks
            int rowNum = 1;
            for (SerializedResult sr : serialized) {
                GroupedResult gr = sr.gr();
    ...
    private static final String[] FIXED_HEADERS =
            {"JsonFileName", "ComponentName", "RuleID", "RuleSeverity", "AffectedFilesCount"};
