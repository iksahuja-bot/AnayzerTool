...
public class WlJBossReportWriter {
    ...
    private static void writeSummarySheet(Workbook wb, StyleSet s,
        ...
        // ── Severity breakdown (single-pass groupBy) ──
        ...
        for (String sev : List.of("CRITICAL", "HIGH", "MEDIUM", "INFO")) {
            ...
            label.setCellStyle(s.severityStyle(sev));
        ...
        List<Map.Entry<String, Long>> byCategory = findings.stream()
                .collect(Collectors.groupingBy(f -> f.rule.category(), Collectors.counting()))
                .entrySet().stream()
                .sorted(Map.Entry.<String, Long>comparingByValue().reversed())
                .toList();
        for (Map.Entry<String, Long> e : byCategory) {
            Row row = sheet.createRow(r++);
    ...
    private static void writeChecklistSheet(Workbook wb, StyleSet s,
        ...
        for (Map.Entry<String, List<WlJBossAnalyzer.Finding>> entry : byCategory.entrySet()) {
            ...
            for (WlJBossAnalyzer.Finding f : deduped.values()) {
                ...
                sevCell.setCellValue(f.rule.severity());
                sevCell.setCellStyle(isStub ? s.stubCell : s.severityStyle(f.rule.severity()));
    ...
    private static int pbStep(Sheet sheet, StyleSet s, int r,
        ...
        headCell.setCellStyle(s.playbookStepBold);
    ...
    private static class StyleSet {
        ...
        // One CellStyle per severity level — built once in the constructor
        ...
        final CellStyle playbookStepBold;      // bold step heading (Col 1)
        ...
        StyleSet(Workbook wb) {
            ...
            // ── Severity styles (one per level, created once) ─────────────────
            for (Map.Entry<String, IndexedColors> e : SEVERITY_COLORS.entrySet()) {
                CellStyle cs = wb.createCellStyle();
                cs.setFillForegroundColor(e.getValue().getIndex());
                cs.setFillPattern(FillPatternType.SOLID_FOREGROUND);
                Font sf = wb.createFont();
                sf.setBold("CRITICAL".equals(e.getKey()));
                cs.setFont(sf);
                sevStyles.put(e.getKey(), cs);
            }
            ...
            // Step bold heading — same as step text but with bold font
            playbookStepBold = wb.createCellStyle();
            playbookStepBold.cloneStyleFrom(playbookStepText);
            Font pbBoldFont = wb.createFont();
            pbBoldFont.setBold(true);
            playbookStepBold.setFont(pbBoldFont);
