...
public class WlJBossAnalyzer {
    ...
    private void printConsoleSummary() {
        long critical = findings.values().stream().filter(f -> "CRITICAL".equals(f.rule.severity())).count();
        long high     = findings.values().stream().filter(f -> "HIGH".equals(f.rule.severity())).count();
        long medium   = findings.values().stream().filter(f -> "MEDIUM".equals(f.rule.severity())).count();
        long info     = findings.values().stream().filter(f -> "INFO".equals(f.rule.severity())).count();
