 ...
 *   2. External properties file in working directory (analyzer.properties)
 *   3. Classpath properties file  (src/main/resources/analyzer.properties)
 *   4. Hard-coded defaults
 *
 * ── Supported CLI arguments ──────────────────────────────────────────────────
 *
 *   --module=<name>          Module to run: upgrade | analyze | merge | wl-jboss26 | wl-jboss27
 *   --input=<path>           Input JAR/WAR/EAR or directory (spring, wl-jboss)
 ...
 *   output.file              Output Excel path
 *
 *   merge.ticket.file        Ticket file (merge module)
 *   merge.component.file     Component file (merge module)
 *
 *   analyzer.resource.folder Report folder (analyze module)
 *   analyzer.parallel.enabled
 *   analyzer.excel.maxCellLength
 *   analyzer.excluded.rules  Comma-separated rule IDs to skip
 *
 *   wl.jar.list.file         JAR list file (wl-jboss module)
 */
public class AppConfig {
    ...
    private void load(String[] args) {
        ...
        // Step 2 – load properties (classpath < working-dir < custom)
        ...
        // Step 3 – parse all CLI args (they take priority over everything)
    ...
    private void loadClasspathProperties() {
        try (InputStream is = AppConfig.class.getClassLoader()
                .getResourceAsStream("analyzer.properties")) {
            if (is != null) {
                Properties p = new Properties();
                p.load(is);
                // lowest priority – only set if key not yet present
                p.forEach((k, v) -> resolved.putIfAbsent(k, v));
            }
    ...
    public static void printHelp() {
        ...
        System.out.println();
        System.out.println("Configuration file (analyzer.properties):");
        System.out.println("  All options above can be set in analyzer.properties using their");
        System.out.println("  property key names. CLI arguments override file values.");
        System.out.println("  The file is loaded from:  ./analyzer.properties  (working directory)");
        System.out.println("                     then:  classpath analyzer.properties (JAR resources)");
        System.out.println();
